    ...
    def load_network(self, net, load_path, strict=True, param_key='params'):
        ...
        if param_key is not None:
            if param_key not in load_net and 'params' in load_net:
                param_key = 'params'
                logger.info(f'Loading: params_ema does not exist, use params. For {net.__class__.__name__}')

            if 'BSRGAN.pth' not in load_path and 'BSRNet.pth' not in load_path and "RealESRGAN+LDL" not in load_path:
                try:
                    load_net = load_net[param_key]
                except:
                    # raise KeyError(f'No key {param_key} in checkpoint file {load_path}')
                    pass
